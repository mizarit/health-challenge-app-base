...
EOT;
  ...
  static public function resize_image($infile, $outfile, $options = array())
  {
    ...
    if (imagepng($image_p, $outfile, $options['quality'])) {
      chmod($outfile, 0777);
      return basename($outfile);
    }
    else {
  ...
  static public function getPresentation($object, $options = array(), $basedir = false)
  {
    ...
    $filename = str_replace(' ', '_', $ar[count($ar)-1]);
    ...
    if (!is_dir("$path/$hash")) {
      if(!@mkdir("$path/$hash", 0777, true)) {
        echo "Error while creating $path/$hash";
      }
    }
    ...
    if (!is_file("$path/$hash/$filename")) {
      ...
      $url = '/'.trim($url, '/');
      @self::resize_image("$basedir$url", "$path/$hash/$filename", $options);
    }
